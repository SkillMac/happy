...
func suitableMethods(typ reflect.Type, reportErr bool) map[string]*methodType {
	...
	for m := 0; m < typ.NumMethod(); m++ {
		...
		hLog.Info(fmt.Sprintf("rpc.RegisterGroup: service: [ %s ], method [ %s ] is registed", typ.Elem().Name(), mname))
	}
	return methods
}
...
func (server *Server) sendResponse(sending *sync.Mutex, req *Request, reply interface{}, codec ServerCodec, errmsg string) {
	...
	if debugLog && err != nil {
		hLog.Info("rpc: writing response:", err)
	}
...
func (c *gobServerCodec) WriteResponse(r *Response, body interface{}) (err error) {
	if err = c.enc.Encode(r); err != nil {
		if c.encBuf.Flush() == nil {
			...
			hLog.Info("rpc: gob error encoding response:", err)
			c.Close()
		}
		return
	}
	if err = c.enc.Encode(body); err != nil {
		if c.encBuf.Flush() == nil {
			...
			// Shut down the connection to signal that the connection is broken.
			hLog.Info("rpc: gob error encoding body:", err)
			c.Close()
		}
		return
	}
...
func (server *Server) Accept(lis net.Listener) {
	for {
		conn, err := lis.Accept()
		if err != nil {
			hLog.Error("rpc.Serve: accept:", err.Error())
			return
		}
		hLog.Debug(conn.RemoteAddr())
...
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		hLog.Info("rpc hijacking ", req.RemoteAddr, ": ", err.Error())
		return
	}
