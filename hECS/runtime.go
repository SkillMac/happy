...
func (runtime *Runtime) SetMaxThread(maxThread int) {
	if runtime.workers != nil && maxThread > 0 {
		...
		hLog.Error(errors.New("max thread must > 0"))
	}
}
	...
	go (func() {
		defer (func() {
			...
			if r != nil {
				err, ok := r.(error)
				if ok {
					hLog.Error(fmt.Sprintf("Failed to execute scheduled task: %s", err.Error()))
				} else {
					hLog.Error(fmt.Sprintf("Failed to execute scheduled task: %s", r))
				}
			}
		})()
