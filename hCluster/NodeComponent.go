...
func (this *NodeComponent) GetRequire() map[*hEcs.Object][]reflect.Type {
...
func (this *NodeComponent) Initialize() error {
	hLog.Info("NodeComponent init .....")
	this.AppName = hConfig.Config.ClusterConfig.AppName
	this.islocationMode = hConfig.Config.ClusterConfig.IsLocationMode
	...
	this.locationGetter()
	hLog.Info("NodeComponent initialized.")
	return nil
}
...
func (this *NodeComponent) InitLocationServerGetter() {
	if !hConfig.Config.ClusterConfig.IsLocationMode {
		return
	}
	locker := sync.RWMutex{}
	isGetting := false
...
func (this *NodeComponent) StartRpcServer() error {
	addr, err := net.ResolveTCPAddr("tcp", hConfig.Config.ClusterConfig.LocalAddress)
	if err != nil {
		return err
	}
	...
	hLog.Info(fmt.Sprintf("NodeComponent RPC server listening on: [ %s ]", addr.String()))
...
func (this *NodeComponent) clientCallback(event string, data ...interface{}) {
	...
	case "close":
		...
		hLog.Info(fmt.Sprintf("  disconnect to remote node: [ %s ]", nodeAddr))
	}
}
...
func (this *NodeComponent) GetNodeFromMaster(role string, selectorType ...SelectorType) (*NodeID, error) {
	...
	client, err := this.GetNodeClient(hConfig.Config.ClusterConfig.MasterAddress)
	if err != nil {
		return nil, err
	}
	var reply *[]*InquiryReply
...
func (this *NodeComponent) ConnectToNode(addr string, callback func(event string, data ...interface{})) (*rpc.TcpClient, error) {
	...
	hLog.Info(fmt.Sprintf("  connect to node: [ %s ] success", addr))
	return client, nil
}
