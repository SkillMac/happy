...
func (this *DbOperate) OpenDB(set_index_func func(ms *mgo.Session)) error {
	hLog.Info(fmt.Sprintf("DbOperate mongodb connect url: %s\n", this.dbcfg.String()))
	...
	hLog.Info(fmt.Sprintf("DbOperate connect %v mongodb...OK", this.dbcfg.String()))
	return nil
}
...
func (this *DbOperate) CloseDB() {
	if this.session != nil {
		...
		this.session.Close()
		this.session = nil
		hLog.Info("Disconnect mongodb url: ", this.dbcfg.String())
	}
}
...
func (this *DbOperate) UpdateInsert(collection string, cond interface{}, doc interface{}) error {
	...
	if err != nil {
		hLog.Error(fmt.Sprintf("UpdateInsert failed collection is:%s. cond is:%v", collection, cond))
	}
...
func (this *DbOperate) RemoveOne(collection string, cond_name string, cond_value int64) error {
	...
	if err != nil && err != mgo.ErrNotFound {
		hLog.Error(fmt.Sprintf("remove failed from collection:%s. name:%s-value:%d", collection, cond_name, cond_value))
	}
...
func (this *DbOperate) RemoveOneByCond(collection string, cond interface{}) error {
	...
	if err != nil && err != mgo.ErrNotFound {
		hLog.Error(fmt.Sprintf("remove failed from collection:%s. cond :%v, err: %v.", collection, cond, err))
	}
...
func (this *DbOperate) RemoveAll(collection string, cond interface{}) error {
	...
	if err != nil && err != mgo.ErrNotFound {
		hLog.Error(fmt.Sprintf("DbOperate.RemoveAll failed : %s, %v", collection, cond))
		return err
	}
	hLog.Debug(fmt.Sprintf("DbOperate.RemoveAll: %v, %v", change.Updated, change.Removed))
	return nil
}
...
func (this *DbOperate) DBFindOne(collection string, cond interface{}, resHandler func(bson.M) error) error {
	...
	if err := q.One(m); err != nil {
		if mgo.ErrNotFound != err {
			hLog.Error(fmt.Sprintf("DBFindOne query falied,return error: %v; name: %v.", err, collection))
		}
		return err
	}
...
func (this *DbOperate) DBFindAll(collection string, cond interface{}, resHandler func(bson.M) error) error {
	...
	for iter.Next(m) == true {
		if nil != resHandler {
			err := resHandler(m)
			if err != nil {
				hLog.Error(fmt.Sprintf("resHandler error :%v!!!", err))
				return err
			}
		}
	}
...
func (this *DbOperate) StrongDBFindAll(collection string, cond interface{}, resHandler func(bson.M) error) error {
	...
	hLog.Debug(fmt.Sprintf("[DbOperate.DBFindAll] name:%s,query:%v", collection, cond))
...
func (this *DbOperate) DBFindAllEx(collection string, cond interface{}, resHandler func(*mgo.Query) error) error {
	...
	hLog.Error(fmt.Sprintf("[DbOperate.DBFindAll] name:%s,query:%v", collection, cond))
...
func (this *DbOperate) WriteGridFile(collection string, filename string, data []byte) error {
	hLog.Debug(fmt.Sprintf("Write grid file: %v, size %v.", filename, len(data)))
	...
	if err != nil {
		hLog.Error("Write grid file fail: ", err)
	}
	return err
}
